...
function confirmSubmit() {
  ...
  return confirm("Are you sure you want to amend this current account?")
}
...
function inputOverdraftLimit(input) {
  const odlimit = parseInt(input.value);
  const value = document.getElementById("accountbal").value;
  // parses the value from the input display
  const bal = parseFloat(value.split("€")[1]);
  ...
  if (value.charAt(0).toUpperCase() == "C" && odlimit <= bal) {
    // alerts the user if the overdraft limit can't contain the balance
    input.setCustomValidity("The overdraft limit of €" + odlimit + " is too low. It can't allow for the current balance of €" + bal);
  } else {
    // a valid overdraft limit was entered, clear any errors on the input
    input.setCustomValidity("");
  }
}
